...
var watchList = [
  ...
  function calcAvarageFunc(arr) {
    let arrNolan = arr.filter((item) => item.Director === "Christopher Nolan");
    let arrMap = arrNolan.map((item) => ({
      imdbRating: +item.imdbRating,
    }));
    let initialValue = 0;
    let avarageReduce = arrMap.reduce(
      (accamulator, currenValue, index) =>
        accamulator + currenValue.imdbRating / arrMap.length,
      initialValue
    );
    return avarageReduce;
  }
  
  console.log(calcAvarageFunc(watchList));